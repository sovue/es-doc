...
**Whitespace is drawn.** Every space in every code panel renders as `∙` in `--code-whitespace` — the muted token colour, never text weight, the way an editor's "show whitespace" and Ren'Py's own linter draw it. In a language where indentation *is* syntax, three spaces where four belong is worth being able to see. It is applied as a Pygments *filter*, re-tagging runs of spaces as `Whitespace` before the formatter sees them, which is the only place the treatment can be uniform: lexers disagree wildly about whether whitespace is a token at all — the stock Python lexer leaves indentation untokenised entirely, the Ren'Py one tags some of its own — and a pass over the highlighted HTML inherits every one of those disagreements.
...
The glyphs make putting the spaces back the price of admission for anything that takes code off the page, so both routes off it — a hand-made selection and the copy button — go through one place, `code.js`, loaded site-wide rather than per template. Code panels appear on docs pages, in the resource browser's viewer and on the warper page, several of them out of markdown no template inspects; a page that has none pays nothing for the script. Anything with a copy path of its own (the viewer's whole-file button in `resources.js`) reuses `window.unglyphCode` rather than keeping a second copy of the glyph — that duplication had already gone stale once, and shipped a viewer whose copy button pasted `import∙random`.
