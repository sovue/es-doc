...
- `dest_path` — путь, куда класть результат. При `in_place` со значением `True` переименовывает `src_path` в `dest_path`
- `rename_non_ascii` — при значении `True` переименовывает файлы в латиницу, транслитерируя кириллицу при наличии.
- `rescale_coordinates` — при значении `True` умножает координаты в коде на 2/3, подгоняя под размеры для Andoid.
...
- `new_virtual_root` — если в ходе портирования требуется добавить такой префикс, он указывается здесь.
...
- `path_rewrite_rules` — правила по перемещению файлов. Cинтаксис: `префикс, шаблон, новый_префикс`.
...
::: warning
Play Market версия Бесконечного Лета очень сильно устарела, по сравнению с ПК-версией, потому некоторый код может не работать. Единственный вариант решения — [установить `APK` с обновлённой версией БЛ.](https://disk.yandex.ru/d/mDaHwXrS74B4Vw)
:::
