...
Хотите разнообразить ваш мод? Придать ему какую-то уникальную черту?
Вы можете попробовать сделать уникальный интерфейс модификации.
...
Данные экраны присутствуют в игре и их можно заменить.
В этом примере мы не будем создавать экраны, предполагется, что у вас есть уже готовые экраны, которые вы хотели бы заменить.

В данном методе мы будем запускать наш мод с лейбла, который заменяет часть экранов и главное меню, после чего мы можем заменить их
обратно, при выходе из меню мода.

Для начала нам нужно обьявить функции замены наших экранов.
...
init python:
    def my_mod_screen_save():#Функция сохранения экранов из оригинала.
        for name in ["main_menu","game_menu_selector","quit","say","preferences","save","load","nvl", 'yesno_prompt']:
            renpy.display.screen.screens[("my_mod_old_"+name, None)] = renpy.display.screen.screens[(name, None)]
    def my_mod_screen_act():#Функция замены экранов из оригинала на собственные.
        ...
        for name in ["main_menu","game_menu_selector","quit","say","preferences","save","load","nvl", 'yesno_prompt']: #Уберите из списка ненужные названия экранов, если не хотите их заменять.
            renpy.display.screen.screens[(name, None)] = renpy.display.screen.screens[(my_mod_+name, None)]
        config.mouse = {'default' : [("images/misc/mouse/1.png",  0, 0)]} #Вставте ваш путь до курсора в вашего мода, если его нет - закомментируйте строку.
        config.main_menu_music = 'mods/my_mod/music/main_menu.mp3' #Вставте ваш путь до музыки в главном меню.
    def my_mod_screens_diact(): #Функция обратной замены.
        ...
        try:
            ...
            config.version = "1.2"
            for name in ["main_menu","game_menu_selector","quit","say","preferences","save","load","nvl", 'yesno_prompt']:
                renpy.display.screen.screens[(name, None)] = renpy.display.screen.screens[("my_mod_old_"+name, None)]
            ...
            renpy.quit()
    #Обьеденяем функцию слхранения экранов и замены в одну.
...
::: warning
В данном случае название ваших экранов должно быть соответствовать виду:
`префикс мода + название экрана в оригинале`.
пример с main_menu:
префикс мода - my_mod,
значит ваш экран должен называться: my_mod_main_menu
:::
...
label my_mod_index:
    ...
    stop music fadeout 3 #Остонавливаем музыку.
